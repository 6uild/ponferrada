...
interface InitDrawerResult {
  ...
  readonly deleteKeyringLink: Element;
...
const initDrawer = async (drawerComponent: React.Component): Promise<InitDrawerResult> => {
  ...
  const [recoveryWordsLink, requestsLink, deleteKeyringLink] = drawerElements;
  ...
  expect(deleteKeyringLink.textContent).toBe("Delete Keyring");
  expect(termsLink.textContent).toBe("Terms & Conditions");

  return { recoveryWordsLink, requestsLink, deleteKeyringLink, termsLink };
};
...
export const clickDeleteKeyring = async (drawerComponent: React.Component): Promise<void> => {
  const { deleteKeyringLink } = await initDrawer(drawerComponent);
  await click(deleteKeyringLink);
};
