...
import { travelToKeyring, whenOnNavigatedToRoute } from "../../utils/test/navigation";
import { withChainsDescribe } from "../../utils/test/testExecutor";
import * as Drawer from "../keyring/test/drawer";
import { DELETE_KEYRING_ROUTE, RECOVERY_WORDS_ROUTE, REQUEST_ROUTE, TERMS_URL } from "../paths";
import { checkCreateAccount, getTransactionsCount } from "./test/operateKeyring";

describe("DOM > Feature > Keyring Status", () => {
  ...
  let keyringStatusDom: React.Component;

  beforeEach(async () => {
    keyringStatusDom = await travelToKeyring(personaMock);
  }, 60000);

  it("redirects to the Recovery Words view when link clicked in Drawer menu", async () => {
    await Drawer.clickRecoveryWords(keyringStatusDom);
  ...
  it("redirects to the Requests view when link clicked in Drawer menu", async () => {
    await Drawer.clickRequests(keyringStatusDom);
    await whenOnNavigatedToRoute(REQUEST_ROUTE);
  }, 60000);

  it("redirects to the Delete Keyring page when Delete keyring was clicked", async () => {
    await Drawer.clickDeleteKeyring(keyringStatusDom);
    await whenOnNavigatedToRoute(DELETE_KEYRING_ROUTE);
  }, 60000);
  ...
  it("redirects to the Terms and Conditions page", async () => {
    ...
    window.open = jest.fn();
    await Drawer.clickTerms(keyringStatusDom);
  ...
  it("has a select dropdown that enables the creation and selection of accounts", async () => {
    const accountInput = TestUtils.findRenderedDOMComponentWithTag(keyringStatusDom, "input");
    expect(accountInput.getAttribute("value")).toBe(ACCOUNT);

    await click(accountInput);
    await checkCreateAccount(keyringStatusDom);
  }, 60000);

  it("has a transactions box with two transactions", () => {
    expect(getTransactionsCount(keyringStatusDom)).toBe(2);
  }, 60000);

  it("has a send transaction box", () => {
    const tx = TestUtils.scryRenderedDOMComponentsWithTag(keyringStatusDom, "li")[1];
    const txTime = tx.children[1].children[1].textContent;
    expect(txTime).toBe(aProcessedTx.time);
  }, 60000);
});

withChainsDescribe("DOM > Feature > Keyring Status", () => {
  it("generates a link inside transaction box for an ethereum transaction", async () => {
    ...
    const keyringStatusDom = await travelToKeyring(personaInfo);

    // Check for the link
    const links = TestUtils.scryRenderedDOMComponentsWithTag(keyringStatusDom, "a");
