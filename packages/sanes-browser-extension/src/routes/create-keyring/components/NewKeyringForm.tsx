...
export const CREATE_KEYRING_ID_STEP_1 = "create-keyring-step1";
...
interface Props {
  readonly onCreateKeyring: (values: FormValues) => void;
  readonly onBack: () => void;
}

const NewKeyringForm = ({ onCreateKeyring, onBack }: Props): JSX.Element => {
  const onSubmit = async (values: object): Promise<void> => {
    const formValues = values as FormValues;
    await onCreateKeyring(formValues);
  };
  ...
  return (
    <NeumaPageLayout id={CREATE_KEYRING_ID_STEP_1} primaryTitle="New" title="Keyring">
