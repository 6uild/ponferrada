...
import { REGISTER_IMAGE_FIELD } from "../../registerArtifact/components";
...
export const registerPersonalizedAddress = async (browser: Browser, page: Page): Promise<string> => {
  await page.click(`#${REGISTER_ARTIFACT_ROUTE.replace("/", "\\/")}`);

  // Fill the form
  const username = `${randomString(10)}*iov`;
  await page.type(`input[name="${REGISTER_IMAGE_FIELD}"]`, username);
