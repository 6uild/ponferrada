...
import Layout, { REGISTER_IMAGE_FIELD } from "./components/index";
import { isValidImageName } from "./index";
...
async function validate(values: object): Promise<object> {
  ...
  const username = formValues[REGISTER_IMAGE_FIELD];
  if (!username) {
    errors[REGISTER_IMAGE_FIELD] = "Required";
    return errors;
  }
  const checkResult = isValidImageName(username);

  switch (checkResult) {
    case "too_short":
      errors[REGISTER_IMAGE_FIELD] = "Personalized address should be at least 3 characters";
      break;
    case "too_long":
      errors[REGISTER_IMAGE_FIELD] = "Personalized address should be maximum 64 characters";
      break;
    case "wrong_chars":
      errors[REGISTER_IMAGE_FIELD] =
